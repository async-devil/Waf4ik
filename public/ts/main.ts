...
let menuSize: number = rnd.width / 3;
//*---------------------------------------------Creating waf4ik---------------------------------------------*/
...
let menu: Menu = new Menu(rnd, '#000b');
menu.x = rnd.width / 2;
menu.y = rnd.height / 2;
...
leftButton2.onclick = () => {
  leftButton2.OnClick();
  if (particlesOnClick > 0) particlesOnClick -= 1.5;
};
